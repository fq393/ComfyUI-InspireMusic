...
class InspireMusicFrontEnd:
    def __init__(self,
        ...
        try:
            ...
            logging.error(f"WavTokenizer loading failed: {e}")
            raise

        try:
            self.model = InspireMusicModel(configs['llm'], configs['flow'], configs['hift'], configs['wavtokenizer'], dtype, fast, fp16)
            logging.info("InspireMusicModel created successfully")
        except Exception as e:
            logging.error(f"InspireMusicModel creation failed: {e}")
            raise
        ...
        try:
            ...
            logging.error(f"Model weights loading failed: {e}")
            raise
